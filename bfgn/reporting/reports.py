...
class Reporter(object):
    ...
    def __init__(
        self, data_container: data_core.DataContainer, experiment: experiments.Experiment, config: configs.Config
    ) -> None:
    ...
    def create_model_report(self) -> None:
        ...
        with PdfPages(filepath_report) as pdf:
            ...
            sampled = samples.Samples(
                self.data_container.training_sequence,
                self.experiment.model,
                self.experiment.config,
                self.experiment.is_model_trained,
                self.data_container.feature_band_types,
                self.data_container.response_band_types,
            ...
            validation_sampled = samples.Samples(
                self.data_container.validation_sequence,
                self.experiment.model,
                self.experiment.config,
                self.experiment.is_model_trained,
                self.data_container.feature_band_types,
    ...
    def plot_network_feature_progression(
        self, sampled: samples.Samples, compact: bool, max_pages: int = None, max_filters: int = None
    ) -> List[plt.Figure]:
    ...
    def plot_sample_histograms(self, sampled: samples.Samples, max_responses_per_page: int = None) -> List[plt.Figure]:
        max_responses_per_page = max_responses_per_page or self.experiment.config.model_reporting.max_responses_per_page
        return samples_viz.plot_sample_histograms(sampled, max_responses_per_page=max_responses_per_page)
    ...
    def plot_regression_deviation(
        ...
        for _r in range(len(self.data_container.response_band_types)):
            ...
            if self.data_container.response_band_types[_r] == "R":
                bounds = self.parity_plot(
                    train_sampled.raw_predictions, train_sampled.raw_responses, plt.subplot(gs1[0, 0]), _r
                )
                plt.subplot(gs1[0, 0]).set_title("Training")
                if val_sampled is not None:
                    self.parity_plot(
                        val_sampled.raw_predictions,
                        val_sampled.raw_responses,
                        plt.subplot(gs1[0, 1]),
                        _r,
                        bounds=bounds,
                    )
    ...
    def parity_plot(
        self, pred_Y: np.array, test_Y: np.array, ax: plt.Axes, response_ind: int, bins: int = 200, bounds=[]
    ):
    ...
    def plot_spatial_error(
        self, sampled: samples.Samples, max_pages: int = None, max_responses_per_page: int = None
    ) -> List[plt.Figure]:
        if self._get_response_data_types() is _LABEL_CATEGORICAL:
