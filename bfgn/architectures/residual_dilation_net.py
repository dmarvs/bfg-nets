...
class ArchitectureConfigSection(
    config_sections.BlockMixin, config_sections.DilationMixin, config_sections.BaseArchitectureConfigSection
):
    pass
...
def create_model(
    ...
    for idx_block, num_subblocks in enumerate(block_structure):
        ...
        for idx_sublayer in range(num_subblocks):
            subblock = Conv2D(filters=filters, dilation_rate=dilation_rate, kernel_size=kernel_size, padding=padding)(
                subblock
            )
            if use_batch_norm:
...
def _add_residual_shortcut(input_layer: keras.layers.Layer, residual_module: keras.layers.Layer):
    ...
    if inshape != residual_shape:
        ...
        shortcut = Conv2D(filters=residual_shape[-1], kernel_size=(1, 1), padding="valid", strides=strides)(shortcut)
