...
def build_training_data_ordered(
    config: configs.Config, feature_raw_band_types: List[str], response_raw_band_types: List[str]
    ...
    feature_memmap_size_gb = (
        n_features * 4 * config.data_build.max_samples * (config.data_build.window_radius * 2) ** 2 / 1024.0 ** 3
    )
    assert (
    ...
    response_memmap_size_gb = (
        n_responses * 4 * config.data_build.max_samples * (config.data_build.window_radius * 2) ** 2 / 1024.0 ** 3
    )
    assert (
                ...
                if len(subset) > 0:
                    if boundary_set is not None:
                        ...
                        for _x_loc in range(len(subset)):
                            ...
                            boundary_fraction = np.sum(
                                np.isnan(bound_dat[:, x_loc : x_loc + config.data_build.loss_window_radius * 2])
                    ...
                    if np.any(valid_locations[subset]):
                        ...
                        col_dat = common_io.read_chunk_by_row(
                            response_sets,
                            all_site_upper_lefts[_site][len(config.raw_files.feature_files[_site])],
                            all_site_xy_sizes[_site][0],
                            config.data_build.loss_window_radius * 2,
                            _row,
                        ...
                        for _x_loc in range(len(subset)):
                            ...
                            response_fractions = np.sum(
                                np.isnan(col_dat[:, x_loc : x_loc + config.data_build.loss_window_radius * 2]),
                                axis=(0, 1),
                            ) / float(config.data_build.loss_window_radius ** 2)
    ...
    _logger.debug(
        "Step through sites in order and grab {} sample from each until max samples".format(num_reads_per_site)
    )
    ...
    features, responses = _open_temporary_features_responses_data_files(config, n_features, n_responses, read_type="r+")
...
def build_training_data_from_response_points(
    ...
    feature_memmap_size_gb = (
        num_features * 4 * config.data_build.max_samples * (config.data_build.window_radius * 2) ** 2 / 1024.0 ** 3
    )
    assert (
        feature_memmap_size_gb < config.data_build.max_built_data_gb
    ), "Expected feature memmap size: {} Gb, limit: {}".format(
        ...
        if config.raw_files.response_files[_site][0].split(".")[-1] in sections.VECTORIZED_FILENAMES:
            ...
            for _sample in tqdm(range(len(vector)), ncols=80, desc="Reading responses"):
                if vector[_sample]["geometry"]["type"] == "Point":
                    ...
                    if (
                        x_sample_points[-1] < lower_coord[0] + resolution[0] * config.data_build.window_radius
                        or x_sample_points[-1] > upper_coord[0] - resolution[0] * config.data_build.window_radius
                        or y_sample_points[-1] < upper_coord[1] + resolution[1] * config.data_build.window_radius
                        or y_sample_points[-1] > lower_coord[1] - resolution[1] * config.data_build.window_radius
                    ):
    ...
    if total_samples > config.data_build.max_samples:
        ...
        total_samples = sum([site_responses.shape[0] for site_responses in responses_per_site])
        _logger.debug(
            "Kept {} total samples across {} sites after discarding".format(total_samples, len(responses_per_site))
        )
    ...
    for _site in range(0, len(config.raw_files.feature_files)):
        ...
        for _cr in tqdm(range(len(xy_sample_locations)), ncols=80, desc="Reading features"):
            ...
            local_feature = read_labeling_chunk(
                _site, local_xy_px_locations, config, reference_subset_geotransforms[_site]
            )
    ...
    responses = np.memmap(
        data_core.get_temporary_responses_filepath(config), dtype=np.float32, mode="w+", shape=(sample_index, 1)
    )
...
def calculate_categorical_weights(
    responses: List[np.array], weights: List[np.array], config: configs.Config, batch_size: int = 100
) -> List[np.array]:
...
def read_labeling_chunk(
    _site: int, offset_from_ul: List[List[int]], config: configs.Config, reference_geotransform: List[float]
) -> np.array:
    ...
    mask = common_io.read_mask_chunk(_site, offset_from_ul.pop(0), window_diameter, geotransform, config)
...
def _open_temporary_features_responses_data_files(
    ...
    _logger.debug(
        "Create temporary munged features data file with shape {} at {}".format(shape_features, features_filepath)
    )
    features = np.memmap(features_filepath, dtype=np.float32, mode=read_type, shape=shape_features)

    _logger.debug(
        "Create temporary munged responses data file with shape {} at {}".format(shape_responses, responses_filepath)
    )
...
def _save_built_data_files(
    features_munged: np.array, responses_munged: np.array, weights_munged: np.array, config: configs.Config
) -> None:
...
def _resize_munged_features(features_munged: np.array, num_samples: int, config: configs.Config) -> np.array:
    ...
    del features_munged
    _logger.debug("Reload data from memmap files with modified sizes; new features shape {}".format(new_features_shape))
...
def _resize_munged_responses(responses_munged: np.array, num_samples: int, config: configs.Config) -> np.array:
    ...
    _logger.debug(
        "Reload data from memmap files with modified sizes; new responses shape {}".format(new_responses_shape)
    )
